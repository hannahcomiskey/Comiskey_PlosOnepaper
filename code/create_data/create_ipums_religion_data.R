...
# Pull out correct columns for each country ----------------------
# tmp <- vector()
# constant_cols <- c(colnames(ipums_data)[c(1:2,4:27)], colnames(ipums_data)[256:270])
# for(i in 1:nrow(surveys_country_codes)) {
#   country_name <- surveys_country_codes$Country[i]
#   if(country_name %in% c("Niger", "Madagascar", "Liberia", "Burundi")) {
#     country_code <- ifelse(country_name == "Niger", "NE",
#                            ifelse(country_name == "Madagascar", "MG",
#                                   ifelse(country_name == "Liberia", "LR", "BI")))
#   } else {
#     country_code <- surveys_country_codes$Code[i]
#   }
#   district_col <- colnames(ipums_data)[grep(paste0("GEO_",country_code), colnames(ipums_data))][1] # pull out distinct column - either merged for multiple years or first for one survey
#   tmp <- c(tmp, district_col) # Checking to match number of countries to number of cols
# }
# 
# data <- ipums_data[, c(constant_cols,tmp)] # only using required columns
...
refined_data <- refined_data %>%
  ...
  select(PSU, DOMAIN, SAMPLE, Country, YEAR, PERWEIGHT, Religion, Wealth_quintiles, Current_source_standard, Current_source_detailed, Method, Afghanistan_subnat:Zimbabwe_subnat)
...
religion <- as.data.frame(prop.table(svytable(~region_country_year + Religion, IPUMSdesign), 1))
religion <- religion %>% separate(region_country_year, c("Region", "Country", "Year"), "_") %>%
  rename(Subnat_Freq=Freq)
saveRDS(as.data.frame(religion), paste(folder, "/","religion_ipums.RDS" , sep=""))

wealth <- as.data.frame(prop.table(svytable(~region_country_year + Wealth_quintiles, IPUMSdesign), 1))
wealth <- wealth %>% separate(region_country_year, c("Region", "Country", "Year"), "_") %>%
  rename(Subnat_Freq=Freq)
saveRDS(as.data.frame(wealth), paste(folder, "/","wealth_quintiles_ipums.RDS" , sep=""))
