...
# Load results  ----------------------------------------------------------------
mod <- readRDS('results/JAGS/JAGS_mod_Bspline_all_parallel.RDS')
...
# grep("a0\\[1,1\\]", vars)
# 
# grep("a\\[195,5,13\\]", vars)
# 
# 
# # a0[m, p] + inprod(B[p, t, 1:num_knots], a[m, p, 1:num_knots])
# 
# # Get alpha_pms 
# alpha_pms <- mod$BUGSoutput$sims.array[,,grep("a0\\[1,1\\]", vars)[1]:grep("a0\\[195,5\\]", vars)[1]]
# alpha_pms <- rbind(alpha_pms[,1,], alpha_pms[,2,], alpha_pms[,3,])
# dim(alpha_pms)
# beta.k <- mod$BUGSoutput$sims.array[,,grep("a\\[1,1,1\\]", vars):grep("a\\[195,5,13\\]", vars)]
...
#       alpha_pms_samp <- alpha_pms[,grep(paste0('a0\\[',p,',',m,'\\]'), colnames(alpha_pms))]
#       beta.k_samp <- beta.k[,grep(paste0('a\\[',p,',',m,','), colnames(beta.k))]
#       for(t in 1:n_years){ 
#         x=n_samps*c
#         i=0
#         for(s in c(x-1999):x){
...
#     } # end M loop
#   } # end P loop 
#   saveRDS(z, paste0('results/JAGS/JAGS_mod_Bspline_z_samps_chain',c,'.RDS'))
#   saveRDS(P, paste0('results/JAGS/JAGS_mod_Bspline_P_samps_chain',c,'.RDS'))
# } # end C loop
...
saveRDS(P_array, 'results/JAGS/P_array_bspline.RDS')
