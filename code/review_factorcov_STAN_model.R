...
traceplot(fit, pars = c("L_d"), inc_warmup = FALSE, nrow=2)
...
L
fit %>%
  spread_draws(`L[1,1]`, `L[2,2]`, `L[2,1]`, `L[2,2]`, `L[3,1]`, `L[3,2]`, `L[4,1]`, `L[4,2]`, `L[5,1]`, `L[5,2]` ) %>%
  summarise_draws()

L_df <- as_tibble(as.vector(L)) %>%
  mutate(Parameter = c(paste0("L[",1:nrow(L), ",", rep(1,5), "]" ), paste0("L[",1:nrow(L), ",", rep(2,5), "]" )))

fit %>%
  spread_draws(`L[1,1]`, `L[2,2]`, `L[2,1]`, `L[2,2]`, `L[3,1]`, `L[3,2]`, `L[4,1]`, `L[4,2]`, `L[5,1]`, `L[5,2]` ) %>%
  pivot_longer(cols = c(`L[1,1]`, `L[2,2]`, `L[2,1]`, `L[2,2]`, `L[3,1]`, `L[3,2]`, `L[4,1]`, `L[4,2]`, `L[5,1]`, `L[5,2]` ), names_to='Parameter', values_to = 'sample') %>%
  ggplot() +
 # stat_halfeye(aes(y = Parameter, x = sample)) +
...
psi
fit %>%
  spread_draws(`psi[1]`, `psi[2]`, `psi[3]`, `psi[4]`, `psi[5]`) %>%
  summarise_draws()

fit %>%
  spread_draws(`psi[1]`, `psi[2]`, `psi[3]`, `psi[4]`, `psi[5]`) %>%
  pivot_longer(cols = c(`psi[1]`, `psi[2]`, `psi[3]`, `psi[4]`, `psi[5]`), names_to='Parameter', values_to = 'sample') %>%
...
L_t
fit %>%
  spread_draws(`L_t[1]`, `L_t[2]`, `L_t[3]`, `L_t[4]`, `L_t[5]`, `L_t[6]`, `L_t[7]`) %>%
  summarise_draws()

# fit %>%
#   spread_draws(`mu_lt[1]`, `mu_lt[2]`, `mu_lt[3]`, `mu_lt[4]`) %>%
#   summarise_draws()

L_t_df <- as_tibble(L_t) %>%
  mutate(Parameter = paste0("L_t[",1:length(L_t),"]"))

fit %>%
  spread_draws(`L_t[1]`, `L_t[2]`, `L_t[3]`, `L_t[4]`, `L_t[5]`, `L_t[6]`, `L_t[7]`) %>%
  pivot_longer(cols = c(`L_t[1]`, `L_t[2]`, `L_t[3]`, `L_t[4]`, `L_t[5]`, `L_t[6]`, `L_t[7]`), names_to='Parameter', values_to = 'sample') %>%
  ggplot() +
  #stat_halfeye(aes(y = Parameter, x = sample)) +
  geom_density(aes(x = sample), fill='grey', alpha=0.8) +
...
colnames(beta_c_df) <- c('value', 'Parameter')
...
alpha_df <- as_tibble(alpha_sim) %>%
  ...
  mutate(index_subnat = rep(1:5, each=5),
    Parameter = paste0("alpha_pms[",rep(1:5, 5),",",rep(1:5, each=5),"]"))
...
random_spline_comp <- Zih %*% delta_k.mean[5,1,]
## Plot basis
par(lwd = 3, cex.axis = 1.3, cex.lab = 1.3, cex.main = 1.3, mfrow = c(1,1))
