...
# ### Function for the Density of a trunc cauchy
# r.tr.cauchy<-function(n,mu,s,A,B) mu + s*tan(runif(n)*((atan((B-mu)/s)-atan((A-mu)/s))) + atan((A-mu)/s))
#
# Sigma_beta_sd <- r.tr.cauchy(n=5, mu=0, s=2.5, A=0, B=5)

sigma_beta_sd_sim <- c(3, 1.5, 3, 2.5, 2) # abs(rcauchy(5, location = 0, scale = 2.5))
...
for(c in 1:ncol(beta_c_sim)) {
  beta_c_sim[,c] <- runif(5, min=0, max=3) # mvtnorm::rmvnorm(1, mean = rep(0,5), sigma=Sigma_beta_sim)
}

# simulate the province-level parameters 
matchcountry <- c(rep(1, 7), rep(2, 4), rep(3, 4), rep(4,5))
# sigma_alpha_sim <- truncnorm::rtruncnorm(5, a=0, b=Inf, mean = 0, sd =2)
sigma_alpha_sim <- c(1, 0.5, 1.2, 1, 0.6)
for(m in 1:M) {
  for(p in 1:P) {
...
sigma_delta_sim <- truncnorm::rtruncnorm(5, a=0, b=Inf, mean = 0, sd =2)
sigma_delta_sim <- rep(0.5, 5)

for(m in 1:M) {
  for(p in 1:P) {
    for(h in 1:(H-1)) {
      delta_sim[m,p,h] <- rnorm(1, mean=0, sd=sigma_delta_sim[m])
    }
...
index_subnat_table <- tibble(index_country = as.character(matchcountry), index_subnat = as.character(1:20))

colnames(P_sim_df) <- c('index_year', 'index_method', 1:20)
P_sim_df <- P_sim_df %>%
  pivot_longer(cols = `1`:`20`, names_to = 'index_subnat', values_to = 'Public') %>%
  rowwise() %>%
...
samps <- tibble(index_country = c(rep(1,5), rep(2,3), rep(3,2), rep(4,5)), index_year = c(4,12, 15, 20, 21, 8, 16, 20, 13, 16, 2, 4, 12, 17, 20)) %>%
...
ggplot() +
  ...
  facet_wrap(~ interaction(index_subnat, Method))
...
ggplot() +
  geom_point(data = P_df %>% filter(index_subnat==indexid), aes(x=index_year, y=Observed, colour=Method, pch=Sector)) +
  geom_line(data = P_df %>% filter(index_subnat==indexid), aes(x=index_year, y=Observed, colour=Method, lty=Sector)) +
  geom_hline(data = alpha_temp, aes(yintercept = invlogit.alpha)) +
  facet_wrap(~index_method)
