...
alpha_varcov <- readRDS('data/simulated_data/mle_cov_alpha.RDS')
alpha_varcov_new <- matrix(NA, 5, 5)
sd_alpha <- c(0.3, 0.2, 0.3, 0.4, 0.5)
corr_alpha <- runif(10, -0.3, 0.3)
od=0
for (j in 1:ncol(alpha_varcov_new)) {
  alpha_varcov_new[j,j] = sd_alpha[j]^2
  for (i in (j+1):nrow(alpha_varcov_new)) {
    od = 1 + od
    alpha_varcov_new[i,j] = sd_alpha[j]*sd_alpha[i]*corr_alpha[od]
    alpha_varcov_new[j,i] = sd_alpha[j]*sd_alpha[i]*corr_alpha[od]
  } 
}
...
for(p in 1:P) {
  alpha_sim[1:M,p] <- mvtnorm::rmvnorm(1, mean = as.vector(unlist(beta_c_new)), sigma = alpha_varcov_new)
}
...
for(m in 1:M){
  for(p in 1:P){
    ...
    for(t in 1:n_years) {
      ...
      Z[m,p,t] = rnorm(1, Z_tmp[m,p,t], 0.5)
      # Proportions
...
index_subnat_table <- tibble(index_country = rep(1, P), index_subnat = as.numeric(1:P))
index_sector_table <- tibble(index_sector = c(1,2), Sector = c('Public', 'Private'))
