...
fit <- stan(
  file = "model/STAN/kstar_NCP_logit_MVN_alpha_N_delta_SE.stan",  # Stan program
  data = inputdata,    # named list of data
  pars = pars,
  iter = 16000,         # total number of iterations per chain
  warmup = 2000,
  thin=7,
  chains=3,
  save_warmup = FALSE,
...
saveRDS(fit, file='results/STAN_model_test_kstar_simdata_NCP_MVN_alpha_N_delta_Kenya.RDS')
