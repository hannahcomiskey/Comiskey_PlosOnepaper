...
# # testing splines ---------------------------------
# Testing the singular value decomposition of D=UΣV^(T), therefore Z=BUΣ^(−1)
...
svd_D <- La.svd(D.hk)
svd_D.hk <- svd_D$u%*%diag(svd_D$d)%*%svd_D$vt
Q.kh <- t(svd_D.hk)%*%solve(svd_D.hk%*%t(svd_D.hk))
Zih <- B.ik%*%Q.kh 
...
saveRDS(fit, file='results/STAN_model_test_zih_simdata_allMVN_SVDZih_sigmay.RDS')
