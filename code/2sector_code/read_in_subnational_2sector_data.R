...
SE_source_data_wide_X <- FP_source_data_wide %>% dplyr::filter(Public.SE<=0.005) # Get obs with two missing sectors
...
if(nrow(SE_source_data_wide_X)>0) {
  for(i in 1:nrow(SE_source_data_wide_X)) {
    ...
    if(length(num.SE0)==1 & length(num.SEnon0)>0) {
      ...
      SE_source_data_wide_X[i,col_index+num.SE0] <- mean.SE # replace Public.SE with mean
    } else {
