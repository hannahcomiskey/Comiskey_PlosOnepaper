...
# Fit model -----------------------------------------
vispath = "visualisations/simulated_data/reg_bspline/STAN/"
  
fit <- readRDS('results/STAN_model_reg_basis_KenyaSim_50years.RDS')
...
# Traceplots
traceplot(fit, pars = c("lambda[1,1,1]", "lambda[2,2,3]", "lambda[3,3,9]", "lambda[4,4,6]", "lambda[5,5,4]", "lambda[6,2,4]"), inc_warmup = FALSE, nrow=6)
traceplot(fit, pars = c("lambda[5,1,1]", "lambda[5,2,3]", "lambda[5,3,9]", "lambda[5,4,6]", "lambda[5,5,4]"), inc_warmup = FALSE, nrow=5)
traceplot(fit, pars = c("lambda[1,5,1]", "lambda[2,5,3]", "lambda[3,5,9]", "lambda[4,5,6]", "lambda[5,5,4]", "lambda[6,5,4]"), inc_warmup = FALSE, nrow=6)
traceplot(fit, pars = c("a[1,5,1]", "a[2,5,3]", "a[3,5,9]", "a[4,5,6]", "a[5,5,4]", "a[6,5,4]"), inc_warmup = FALSE, nrow=6)
traceplot(fit, pars = c("cstar"), inc_warmup = FALSE, nrow = 5)
traceplot(fit, pars = c("sigma_tau"), inc_warmup = FALSE, nrow = 5)
...
ggsave(filename = "p_plot_Kenyasim_50years.pdf", path = vispath, height=12, width=15) 
