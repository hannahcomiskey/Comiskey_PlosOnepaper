...
vispath='visualisations/simulated_data/reg_bspline/JAGS/50years/'

# fit_sim model -----------------------------------------
fit_sim <- readRDS('results/JAGS/JAGS_model_KenyaSim_reg_Bspline_50years.RDS')
...
# Plot traceplots -------------------------------------------------------------- 
mcmc_trace(jags.fit.mcmc, pars= c('beta_c[1]', 'beta_c[2]', 'beta_c[3]', 'beta_c[4]', 'beta_c[5]'))
...
# mcmc_trace(jags.fit.mcmc, pars= c('lambda_tilde[1,1,1]', 'lambda_tilde[1,1,2]', 'lambda_tilde[1,1,3]', 'lambda_tilde[1,1,4]', 'lambda_tilde[1,1,5]'))
...
ggsave(filename = "betac_plot_Kenyasim_50years_bspline.pdf", path = vispath, height=12, width=15) 
...
alpha_df <- as_tibble(alpha_sim) %>%
  ...
  pivot_longer(cols=c(`1`:`5`), names_to = 'index_method', values_to ='value') %>%
  rowwise() %>%
  mutate(Parameter = paste0("a0[", index_subnat,",",index_method,"]"))
...
jags.fit.mcmc %>%
  ...
  facet_wrap(~Parameter)
ggsave(filename = "a0_plot_Kenyasim_50years_bspline.pdf", path = vispath, height=12, width=15) 
...
pdf(paste0(vispath, "posterior_predictive_observed_Kenyasim_50years_bspline.pdf")) 
plot(y_sim_mean, y_obs)
abline(a=0, b=1, col='red')
dev.off()

pdf(paste0(vispath,"posterior_predictive_check_Kenyasim_50years_bspline.pdf"))
...
# saveRDS(P_samps_df, file='results/JAGS_model_Kenya_Bspline_50years_Psamps_df.RDS')
...
ggsave(filename = "p_plot_Kenyasim_sum0_50years_bspline.pdf", path = vispath, height=12, width=15) 
...
ggplot() +
  ...
  facet_wrap(~interaction(Method, index_subnat), ncol = 5)
ggsave(filename = "p_plot_Kenyasim_50years_withAlpha_bspline.pdf", path = vispath, height=12, width=15) 
