...
parameters {   // The parameters accepted by the model. 
  ...
  vector[M_count] delta_alpha[P_count]; // variation for beta parameters
...
transformed parameters { 
  ...
  for(c in 1:C_count){
    beta_c[c, 1:M_count] =  diag_pre_multiply(sigmabeta_tau, sigmabeta_Omega)*delta_betac[c, 1:M_count];  
  }
  for(p in 1:P_count){
    alpha_pms[p,1:M_count] = beta_c[matchcountry[p], 1:M_count] +  diag_pre_multiply(sigmaalpha_tau, sigmaalpha_Omega)*delta_alpha[p, 1:M_count];  
    for(m in 1:M_count){ 
      // Spline coefficients
...
model { 
  ...
  sigmabeta_Omega ~ lkj_corr_cholesky(2);
  sigmaalpha_tau ~ normal(0,2);
  sigmaalpha_Omega ~ lkj_corr_cholesky(2);
